...
from chromecompanion.helpers.basehelper import basehelper

class xmltv(basehelper):
    def __init__(self, content_id, datafiles):
        self.program = None
        tz = timezone('Europe/Copenhagen')
        now = datetime.now(tz)
        self.nextStart = now + timedelta(seconds=600)
        try:
            tree = ET.parse('{0}/{1}.xml'.format(datafiles, content_id))
            root = tree.getroot()
